---
title: "When Experience Meets Environment: Professional Backgrounds, Court Composition, and Decision-Making in Immigration Court"
sort: 2
author:
  - name: Sara Heridia
    url: https://sara-heridia.github.io/
    orcid: 0000-0002-0270-681103
    affiliation: Department of Political Science, Emory University
  - name: Matthew Martin
    url: https://mattjamesmartin.github.io/
    orcid: 0000-0003-3460-3603
    affiliation: Department of Government, University of Texas at Austin
categories:
  - Immigration judges
  - Judicial decision-making
  - Professional backgrounds
pub-info:
  status: "Under review"
  blurb: >-
    Analyzing more than 520,000 decisions by 502 immigration judges from 2015–2018, we disaggregate professional backgrounds by domain and examine how court composition moderates individual effects. Immigration-specific experience matters: former ICE attorneys grant relief at lower rates than other prosecutors; nonprofit immigration defense attorneys grant at higher rates than other defense attorneys. These background effects, however, are conditional on context—most pronounced in professionally heterogeneous courts and attenuating in prosecutor-dominated courts, where local norms absorb or suppress prior socialization.
  links:
  - name: Draft
    url: when-experience-meets-environment.pdf
    local: true
    icon: fa-solid fa-file-pdf
---

## Abstract

Research on judicial behavior has long recognized that professional backgrounds shape decision-making, but often treats these effects as uniform across institutional contexts. Focusing on U.S. immigration courts, we argue that understanding how professional experience influences judges requires attention to the specificity of prior socialization and the courts where judges currently sit. Analyzing more than 520,000 decisions by 502 immigration judges from 2015--2018, we disaggregate backgrounds by domain and examine how court composition moderates individual effects. Immigration-specific experience matters: former ICE attorneys grant relief at lower rates than other prosecutors; nonprofit immigration defense attorneys grant at higher rates than other defense attorneys. These background effects, however, are conditional on context — they are most pronounced in professionally heterogeneous courts and attenuate in prosecutor-dominated courts, where local norms absorb or suppress prior socialization. Professional background shapes judicial heuristics, but institutional environment determines whether those heuristics distinguish judges from their peers. {{< fa blank >}}
